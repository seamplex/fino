...
A tensile test specimen of nominal cross-sectional area\ $A = 20~\r{mm} \times 5~\r{mm} = 100~\r{mm}^2$ is fully fixed on one end (magenta surface) and a tensile load of $F_x = 10~\r{kN}$ is applied at the other end (green surface). The Young modulus is\ $E=200~\r{GPa}$ and the Poisson’s ratio is\ $\nu=0.35$.


![Tensile test specimen CAD from CAEplex <https://caeplex.com/p/41dd1>](000-tensile-test/tensile-test-cad.png){#fig:tensile-cad width=12cm}
...
```{.c style=c}
include(000-tensile-test/tensile-test.geo)
```

Out of the six lines, the first three are used to read the CAD file, to se the characteristic element size\ $\ell_c = 1.5~\r{mm}$ and to ask for second-order 10-noded tetrahedra (default first-order 4-node). The last three lines define one physical group each:
...
![Numerical ids of the surfaces in the original CAD.](000-tensile-test/tensile-faces.svg){#fig:tensile-faces width=12cm}

![Three-dimensional mesh with uniform\ $\ell_c=1.5~\r{mm}$ and $\sim$ 50k nodes.](000-tensile-test/tensile-mesh.png){#fig:tensile-mesh width=12cm}
...
```{.fino style=fino}
include(000-tensile-test/tensile-test.fin)
```
 ...
 * It has already been said that the output is 100% controlled by the user. Yet this fact includes not just what is written but also how: the precision of the printed results with [`printf` format specifiers](https://en.wikipedia.org/wiki/Printf_format_string). Note the eight decimal positions in the evaluation of $\sigma_1$ at the origin, whilst the expected value was $100~\r{MPa}$ (the load is $F_x=10^4~\r{N}$ and the cross-sectional area is $100~\r{mm}^2$).
...
![Tensile test results obtained by Fino and post-processed by ParaView. Displacements are warped 500\ times.](000-tensile-test/tensile-test.png){#fig:tensile-vtk width=12cm}
 ...
 * The principal stress $\sigma_1$ should be equal to $F_x/A$, where
     * $F_x=10^4~\r{N}$, and
     * $A = 20~\r{mm} \times 5~\r{mm} = 100~\r{mm}^2$.
     
   In effect, $\sigma_1(0,0,0) = 100~\r{MPa}$.
...
![FreeCAD showing the results obtained with CalculiX in the FEM workbench.](000-tensile-test/freecad-ccx.png){width=12cm #fig:freecad-ccx}
...
![Simscale web interface showing the results obtained with Code-Aster.](000-tensile-test/simscale.png){width=12cm #fig:simscale}
divert(0)
...
The following input files ask Fino to perform a parametric run on $c \in [1:10]$ which controls the characteristic element size\ $\ell_c=10~\r{mm}/c$:

```{.fino style=fino}
include(000-tensile-test/parametric-energy.fin)
```

Depending on the command-line argument `$1`, it includes either `load.fin`

```{.fino style=fino}
include(000-tensile-test/load.fin)
```
or `displ.fin`

```{.fino style=fino}
include(000-tensile-test/displ.fin)
```
...
![Total strain energy\ $U$ computed by Fino as a function of the number of nodes for load and displacement-driven problems.](000-tensile-test/energy.svg){#fig:energy width=12cm}
...
```{.fino style=fino}
include(000-tensile-test/parametric-solver.fin)
```
...
::: {#fig:solver}
![High-end desktop computer](000-tensile-test/tom.svg){#fig:tom width=12cm}

![Virtual server running on the cloud](000-tensile-test/vps.svg){#fig:vps width=12cm}
...
![Wall time for strong parallel scaling test.](000-tensile-test/strong.svg){#fig:strong width=12cm}
